...
const directoryTree = computed<FileTreeNode[]>(() => {
  ...
  return [{ children: categoryNodes, key: 'all', title: '全部文件' }];
});
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    if (selectedTreeKey.value !== 'all') {
      const categoryPrefix = `category:${item.category || 'default'}`;
      const parentKey = `path:${item.category || 'default'}:${normalizePath(item.parentPath)}`;
      ...
      const selectedPathPrefix = selectedTreeKey.value.startsWith('path:')
        ? selectedTreeKey.value.replace(`path:${item.category || 'default'}:`, '')
        : '';
      ...
      const inTree =
        ...
        (selectedTreeKey.value.startsWith(`path:${item.category || 'default'}:`) &&
...
function buildDirectoryChildren(category: string, parentPath: string, paths: Set<string>): FileTreeNode[] {
  ...
  const fileNodes = items.value
    .filter((item) => item.category === category && normalizePath(item.parentPath) === parentPath)
...
function resolveUploadPayload() {
  if (selectedTreeKey.value === 'all') {
    return { category: 'default', parentPath: null };
  }
  ...
  return {
    category: currentFile?.category || 'default',
    parentPath: currentFile?.parentPath || null,
  };
}
