...
import { Alert, Button, Card, Col, Empty, Input, Row, Statistic, Table, Tag } from 'ant-design-vue';
...
<template>
  <Page description="审计日志按最近 200 条记录展示平台请求留痕，支持按关键字筛选。" title="审计日志">
    <section class="platform-audit">
      <Alert
        banner
        message="当前页面读取的是 ABP 审计日志表（数据库留痕），不是解析 `Logs/logs.txt` 这类文件日志。若页面为空，通常表示当前环境尚未产生可用审计记录。"
        type="info"
      />
