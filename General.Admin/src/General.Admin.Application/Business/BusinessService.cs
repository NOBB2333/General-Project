...
public class BusinessService : ITransientDependency
{
    private readonly ICurrentUser _currentUser;
    private readonly CurrentUserDataScopeService _dataScopeService;
    private readonly IRepository<BusinessBudgetExecution, Guid> _budgetExecutionRepository;
    ...
    private readonly IRepository<BusinessForecastHistory, Guid> _forecastHistoryRepository;
    private readonly IRepository<BusinessProcurement, Guid> _procurementRepository;
    private readonly IRepository<BusinessReceivable, Guid> _receivableRepository;
    ...
    public BusinessService(
        ...
        IRepository<BusinessBudgetExecution, Guid> budgetExecutionRepository,
        IRepository<BusinessChain, Guid> businessChainRepository,
        IRepository<BusinessContract, Guid> businessContractRepository,
        IRepository<BusinessForecastHistory, Guid> forecastHistoryRepository,
        IRepository<BusinessProcurement, Guid> procurementRepository,
        IRepository<BusinessReceivable, Guid> receivableRepository,
    ...
    public async Task<BusinessOverviewDto> GetOverviewAsync()
    {
    ...
    public async Task<List<BusinessProjectSummaryDto>> GetProjectListAsync(BusinessProjectListInput input)
    {
        var context = await BuildContextAsync();
    ...
    public async Task<BusinessProjectDetailDto> GetProjectDetailAsync(Guid projectId)
    {
        var context = await BuildContextAsync();
        ...
        return new BusinessProjectDetailDto
        {
            BudgetExecutions = context.BudgetExecutions
                .Where(item => item.ProjectId == projectId)
                .OrderBy(item => item.SortOrder)
                .Select(item => new BusinessBudgetExecutionItemDto
                {
            ...
            BusinessChains = context.BusinessChains
                ...
                .Select(item => new BusinessChainItemDto
                {
            ...
            ForecastHistories = context.ForecastHistories
                ...
                .Select(item => new BusinessForecastHistoryItemDto
                {
            ...
            Procurements = context.Procurements
                .Where(item => item.ProjectId == projectId)
                .OrderBy(item => item.SignDate)
                .Select(item => new BusinessProcurementItemDto
                {
            ...
            Receivables = context.Receivables
                ...
                .Select(item => new BusinessReceivableItemDto
                {
                    Id = item.Id,
    ...
    public async Task<BusinessReportDto> GetReportAsync(BusinessReportQueryInput input)
    {
        var context = await BuildContextAsync();
        var keyword = input.Keyword?.Trim();
    ...
    private static List<BusinessFocusItemDto> BuildFocusItems(List<BusinessWarningItemDto> warnings)
    {
        return warnings
            .Take(4)
            .Select(item => new BusinessFocusItemDto
        {
    ...
    private List<BusinessWarningItemDto> BuildWarningItems(List<BusinessProjectSummaryDto> summaries, BusinessContext context)
    {
        var warnings = new List<BusinessWarningItemDto>();
        ...
        foreach (var summary in summaries)
        {
            ...
            if (summary.GrossProfitRate.HasValue && summary.GrossProfitRate.Value < 25m)
            {
                warnings.Add(BuildWarningItem(
                    ...
                    summary,
                    type: "利润风险",
                    level: summary.GrossProfitRate.Value < 15m ? ProjectPriorities.High : ProjectPriorities.Medium,
                    actionTab: "profit",
                ...
                || summary.BusinessCloseStatus.Contains("待回款", StringComparison.OrdinalIgnoreCase))
            {
                warnings.Add(BuildWarningItem(
                    ...
                    summary,
                    type: "收尾风险",
                    level: summary.BusinessCloseStatus.Contains("待回款", StringComparison.OrdinalIgnoreCase) ? ProjectPriorities.High : ProjectPriorities.Medium,
    ...
    private BusinessWarningItemDto BuildWarningItem(
        BusinessContext context,
        ProjectEntity project,
        BusinessProjectSummaryDto summary,
        string type,
        string level,
        string actionTab,
        string description)
    {
        return new BusinessWarningItemDto
        {
            ActionTab = actionTab,
    ...
    private static int ResolveWarningRank(string level)
    {
        return level switch
        {
            var l when l == ProjectPriorities.High   => 3,
            var l when l == ProjectPriorities.Medium => 2,
            _ => 1
        };
    }
    ...
    private sealed record BusinessContext(
        List<BusinessBudgetExecution>    BudgetExecutions,
        List<BusinessChain>              BusinessChains,
        List<BusinessContract>           Contracts,
        List<BusinessForecastHistory>    ForecastHistories,
        Dictionary<Guid, string>                 OrganizationUnitNameMap,
        List<BusinessProcurement>        Procurements,
        Dictionary<Guid, string>                 ProjectCodeMap,
        Dictionary<Guid, string>                 ProjectNameMap,
        List<ProjectEntity>              Projects,
        List<BusinessReceivable>         Receivables,
        Dictionary<Guid, string>                 UserNameMap)
    {
        // O(1) 单条项目查找
        public Dictionary<Guid, ProjectEntity> ProjectById { get; } =
            Projects.ToDictionary(p => p.Id);

        // 预分组字典：O(1) 按项目取子集
        public Dictionary<Guid, List<BusinessContract>>        ContractsByProject        { get; } = Contracts.GroupBy(c => c.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
        public Dictionary<Guid, List<BusinessProcurement>>     ProcurementsByProject     { get; } = Procurements.GroupBy(p => p.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
        public Dictionary<Guid, List<BusinessReceivable>>      ReceivablesByProject      { get; } = Receivables.GroupBy(r => r.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
        public Dictionary<Guid, List<BusinessBudgetExecution>> BudgetExecutionsByProject { get; } = BudgetExecutions.GroupBy(b => b.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
        public Dictionary<Guid, List<BusinessChain>>           BusinessChainsByProject   { get; } = BusinessChains.GroupBy(b => b.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
        public Dictionary<Guid, List<BusinessForecastHistory>> ForecastHistoriesByProject { get; } = ForecastHistories.GroupBy(f => f.ProjectId).ToDictionary(g => g.Key, g => g.ToList());
    ...
    private BusinessContractItemDto MapContract(BusinessContract item, BusinessContext context, bool canViewSensitiveAmounts)
    {
        return new BusinessContractItemDto
        {
            Amount = MaskAmount(item.Amount, canViewSensitiveAmounts),
    ...
    private BusinessProjectSummaryDto MapProjectSummary(ProjectEntity project, BusinessContext context, bool canViewSensitiveAmounts, bool canViewProfitAnalysis)
    {
        ...
        return new BusinessProjectSummaryDto
        {
    ...
    {
        return _currentUser.IsInRole(PlatformRoleNames.Admin)
            || _currentUser.IsInRole(PlatformRoleNames.Pmo)
            || _currentUser.IsInRole(PlatformRoleNames.Viewer)
            || _currentUser.IsInRole(PlatformRoleNames.Pm);
    }
    ...
    private static string ResolveBusinessCloseStatus(ProjectEntity project, List<BusinessReceivable> receivables)
    {
    ...
    private static bool IsClosed(string status) => ProjectStatuses.IsClosed(status);
}
