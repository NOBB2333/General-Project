...
public class PlatformUserActivityService : ITransientDependency
{
    ...
    public PlatformUserActivityService(
    ...
    public async Task MarkLoginAsync(Guid userId)
    {
        ...
        await SetCacheAsync(userId, new PlatformUserActivityCacheItem
        {
    ...
    private async Task<PlatformUserActivityCacheItem?> GetCacheAsync(Guid userId)
    {
        var json = await _distributedCache.GetStringAsync(GetCacheKey(userId));
        return string.IsNullOrWhiteSpace(json)
            ? null
            : JsonSerializer.Deserialize<PlatformUserActivityCacheItem>(json, JsonSerializerOptions);
    }

    private async Task<PlatformUserActivityCacheItem> LoadCacheAsync(Guid userId)
    {
        var profile = await _userProfileRepository.FindAsync(x => x.UserId == userId);
        var cacheItem = new PlatformUserActivityCacheItem
        {
            ForceLogoutAfter = profile?.ForceLogoutAfter,
            LastSeenAt = profile?.LastSeenAt
        };
    ...
    private async Task SetCacheAsync(Guid userId, PlatformUserActivityCacheItem item)
    {
    ...
    private sealed class PlatformUserActivityCacheItem
    {
