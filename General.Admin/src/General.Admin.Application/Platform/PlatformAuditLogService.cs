...
public class PlatformAuditLogService : ITransientDependency
{
    ...
    public async Task RecordPageVisitAsync(string? userName, string? tenantName, PlatformPageVisitInput input)
    {
    ...
    public async Task<List<PlatformAuditLogItemDto>> GetListAsync(PlatformAuditLogQueryInput input)
    {
        ...
        if (requestLogs.Count > 0)
        {
            ...
            return new PlatformLogDashboardDto
            {
                ...
                TopApis = apiLogs
                    ...
                    .Take(8)
                    .Select(x => new PlatformLogStatItemDto { Count = x.Count(), Key = x.Key, Label = x.Key })
                    .ToList(),
    ...
    private static List<PlatformAuditLogItemDto> FilterByCategory(List<PlatformAuditLogItemDto> logs, string? category)
    {
        return category switch
        {
    ...
    private static PlatformAuditLogItemDto MapItem(AuditLog x)
    {
        return new PlatformAuditLogItemDto
        {
            Id = x.Id,
            ActionSummary = x.Actions
                .OrderByDescending(action => action.ExecutionTime)
    ...
    private async Task<List<PlatformAuditLogItemDto>> GetRequestLogsAsync(PlatformAuditLogQueryInput input)
    {
        var logs = (await _requestAuditStore.QueryAsync(input))
            .Select(item => new PlatformAuditLogItemDto
            {
