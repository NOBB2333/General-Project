...
public class PhaseOneRequestAuditFlushBackgroundService : BackgroundService
{
    private const int BatchSize = 100;
    private static readonly TimeSpan BatchWindow = TimeSpan.FromMilliseconds(250);
    private bool _auditTableMissingLogged;

    private readonly ILogger<PhaseOneRequestAuditFlushBackgroundService> _logger;
    private readonly PhaseOneRequestAuditQueue _queue;
    private readonly IServiceScopeFactory _serviceScopeFactory;

    public PhaseOneRequestAuditFlushBackgroundService(
        ILogger<PhaseOneRequestAuditFlushBackgroundService> logger,
    ...
    {
        var batch = new List<PhaseOneRequestAuditEntry>(BatchSize);
    ...
    private async Task FlushAsync(List<PhaseOneRequestAuditEntry> batch, CancellationToken cancellationToken)
    {
        if (batch.Count == 0)
        {
            return;
